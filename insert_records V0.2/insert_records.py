...
try:
    ...
    cur.execute('create table if not exists pp (city_of_birth varchar(50), first_name varchar(20), last_name varchar(20),\
    ...
    for patient_record in dictinfo:
        ...
        for key in patient_record:
            ...
            if (type(key)==unicode):
                key.encode('utf-8');
            if (type(patient_record[key])==unicode):
                patient_record[key].encode('utf-8');
            #record_list.append(record_value)
        sql_2 = "insert into pp values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        param=patient_record
        print len(param)
        cur.executemany(sql_2,param)  
        #cur.execute('insert into pp values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',record_list)
        conn.commit()
